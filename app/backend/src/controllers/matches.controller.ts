...
export default class MatchesController {
  static getAllMatches(req: Request, res: Response): Promise<Response> {
    ...
    if (inProgress) {
      return MatchesService.getMatchesByStatus(inProgress as string).then(
        ({ status, data }) => res.status(mapStatusHTTP(status)).json(data),
      );
    }

    return MatchesService.getAllMatches().then(({ status, data }) =>
  ...
  static updateMatchByStatus(req: Request, res: Response): Promise<Response> {
    ...
    return MatchesService.updateMatchByStatus(id).then(({ status, data }) =>
      res.status(mapStatusHTTP(status)).json(data));
  }
  ...
  static updateMatchById(req: Request, res: Response): Promise<Response> {
    ...
    return MatchesService.updateMatchById(
      id,
      homeTeamGoals,
      awayTeamGoals,
  ...
  static createMatch(req: Request, res: Response): Promise<Response> {
    return MatchesService.createMatch(req.body).then(({ status, data }) =>
      res.status(mapStatusHTTP(status)).json(data));
  }
}
